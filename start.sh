...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/psc-statement-delta-consumer/env

    PORT="${PSC_STATEMENT_DELTA_CONSUMER_PORT}"
else
    PORT="$1"
    CONFIG_URL="$2"
    ENVIRONMENT="$3"
...
exec java ${JAVA_MEM_ARGS} -jar -Dserver.port="${PORT}" "${APP_DIR}/psc-statement-delta-consumer.jar"
